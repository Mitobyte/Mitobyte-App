/**
 * AI Event Request Parser API Endpoint
 * Uses Cloudflare Workers AI to parse natural language event descriptions
 * for regular users submitting event requests
 */
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  try {
    ...
    const { description: userInput } = body;
...
Extract and return ONLY a valid JSON object with these exact fields:
{
  ...
  "expected_attendees": number or null (extract if mentioned, otherwise null),
...
- Extract expected_attendees if mentioned (e.g., "expecting about 30 people" -> 30)
